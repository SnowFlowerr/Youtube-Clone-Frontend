...
export default function Shorts() {
    ...
    useEffect(() => {
        async function fetchData1() {
            try {
                if (id !== "url" && id) {
                    const vidData = await axios.get(`https://video-streaming-app-backend-production.up.railway.app/api/shorts/${id}`)
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const videosData = await axios.get(`https://video-streaming-app-backend-production.up.railway.app/api/shorts/?limit=${5}&skip=${0}`)
